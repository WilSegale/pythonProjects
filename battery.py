...
if battery.power_plugged == False:
    batteryPulgedIn = f"{BRIGHT}{YELLOW}Power plugged in: {RED}{battery.power_plugged}{RESET}"

else:
    batteryPulgedIn = f"{BRIGHT}{YELLOW}Power plugged in: {GREEN}{battery.power_plugged}{RESET}"
'''
if battery:
    battery = psutil.sensors_battery()
    ...
    if battery.percent >= int(50):
        ...
        print(f"{BRIGHT}{YELLOW}Battery percentage: {YELLOW}***{RED}{battery.percent}%{YELLOW}***{RESET}")
